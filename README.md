...
This project is an in-vehicle car error decoder designed to read vehicle diagnostic data and display useful information to the driver through a projected heads-up display interface. The goal of the system is to create a compact device that can communicate with a vehicle through the OBD-II port, decode error codes, and present warning messages in an easy-to-understand format.
